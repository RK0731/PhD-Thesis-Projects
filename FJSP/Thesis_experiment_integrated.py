...
class shopfloor:
    def __init__(self, env, span, m_no, wc_no, **kwargs):
        ...
        if 'DRL_AS' in kwargs and kwargs['DRL_AS']:
            ...
            kwargs['DRL_R'] = True
            kwargs['AS'] = True
        if 'DRL_MR' in kwargs and kwargs['DRL_MR']:
        ...
        if 'MR' in kwargs and kwargs['MR']:
            print("---> MR mode ON <---")
            self.sequencing_brain = validation_S.DRL_sequencing(self.env, self.m_list, self.job_creator, self.span, \
            show = 0, MR_validated = 1, reward_function = '')
...
DRLs = ['DRL_R',"AS","MR",'DRL_AS','DRL_MR']
title = [x[0]+'+'+x[1] for x in benchmark[:-2]] + ['GP1','GP2'] + ['RA','AS','MR',"Integrated AS+RA","Integrated MR+RA"]
print(title)
...
for run in range(iteration):
    ...
    for idx,rule in enumerate(benchmark):
        ...
        spf = shopfloor(env, span, m_no, wc_no, sequencing_rule = rule[0], routing_rule = rule[1], seed = seed)
        spf.simulation()
    ...
    for idx,x in enumerate(DRLs):
        ...
        env = simpy.Environment()
        exec('spf = shopfloor(env, span, m_no, wc_no, {} = True, seed = seed)'.format(x))
        spf.simulation()
        output_time, cumulative_tard, tard_mean, tard_max, tard_rate = spf.job_creator.tardiness_output()
        sum_record[run].append(cumulative_tard[-1])
        max_record[run].append(tard_max)
