...
class shopfloor:
    def __init__(self, env, span, m_no, wc_no, **kwargs):
        ...
        if len(kwargs):
            ...
            order = "self.routing_brain = validation_R.DRL_routing(self.env, self.job_creator, self.wc_list, {})".format(arch)
            print("---> {} <---".format(arch))
            exec(order)
...
#arch_set = ['ext_validated',"ext_validated"]
arch_set = ['validated']

#arch_set = ['validated',"Norm"]
title = [x for i,x in enumerate(arch_set)]
...
for run in range(iteration):
    ...
    for idx,x in enumerate(arch_set):
        ...
        spf = shopfloor(env, span, m_no, wc_no, arch = x, seed = seed)
        spf.simulation()
...
if export_result:
    ...
    #print(df_max)
    address = sys.path[0]+'\\Thesis_result_figure\\R_RAW_{}wc_tournament.xlsx'.format(wc_no)
